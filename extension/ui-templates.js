...
// 역할: UI HTML 및 반응형 CSS (자동 숨김 스타일 추가됨)
  ...
  function lyricsSettingsHTML() {
      ...
      return `
        <div id="cp-lyrics-panel" class="sep-lyrics-settings" style="display:none;">
            ...
            <div class="sep-ls-row">
                <label>병합(Gap) <span id="val-merge" style="float:right; color:#3ea6ff;">0.05s</span></label>
                <input type="range" id="ap-cfg-merge" class="sep-ls-range" min="0.0" max="1.5" step="0.05" value="0.05">
            </div>
        </div>
      `;
  }
    ...
    return `
      <style>
        ...
        #yt-custom-player-ui {
            ...
            transition: opacity 0.3s ease, transform 0.3s ease; /* transition 수정 */
        }

        /* [New] 자동 숨김 상태 클래스 */
        #yt-custom-player-ui.ui-idle {
            opacity: 0 !important;
            pointer-events: none;
        }
        ...
        .sep-ls-btn:hover { background: #666; }
        ...
        #yt-custom-player-ui.fs-mode .sep-track-group:hover { transform: scale(1.05); border-color: rgba(255,255,255,0.3); }
